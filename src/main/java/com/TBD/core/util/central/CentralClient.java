...
public final class CentralClient
{
	...
	public static String decrypt(String cipherAuthorizationId, String toBeDecrypted) throws Exception
	{
		...
		try
		{
			...
			String cipherURL = String.format(CIPHER_URL, 
									     ...
									     URLEncoder.encode(toBeDecrypted, "UTF-8"));			
		...
		{
			throw new Exception("Unable to retrieve Decipher : " + response.toString());
		}
	...
	private static String determineCentralHost() throws Exception
	{
		...
		centralHost = SystemHelper.getHostName();
		if (!SystemHelper.UNKNOWN_HOST.equals(centralHost))
		{
			if (centralHost.contains("ip-")) //AWS boxes are being given names ip-172-31-35-139.ec2.internal
			{
				/**
			...
			{
				/**
				 * Running from out the cloud / development boxes, IP address of
				 * which is 52.23.185.3
				 */
